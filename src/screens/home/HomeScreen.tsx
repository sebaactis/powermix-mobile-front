		...
		async (isRefresh: boolean = false) => {
			try {
				...
				if (!res.success || !res.data) {
					throw new Error(res.error?.message || "Error al traer el usuario");
				}
			...
			>
				...
				<Text style={styles.message}>
					{userHome?.stampsCounter < 0
						? ProgressMessages.NONE
						: userHome?.stampsCounter < 3
							? ProgressMessages.LOW
							: ProgressMessages.HIGH}
				</Text>

				<View style={styles.btnContainer}>
					<Pressable
						style={[styles.button, { backgroundColor: CARD_BG }]}
						onPress={() =>
					...
					{loading ? null : proofs?.length === 0 ? (
						<View style={styles.noProofsContainer}>
							<Icon
								...
								color="#9e9e9e"
							/>
							<Text style={styles.noProofsText}>
								Aun no cargaste ningún comprobante
							</Text>
						</View>
					) : (
